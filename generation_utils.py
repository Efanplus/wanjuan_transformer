...
class GenerationMixin:
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        print(__class__, "Generating...")
        print(__class__, "inputs: {}".format(inputs) )
        bos_token_id = bos_token_id if bos_token_id is not None else self.config.bos_token_id
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        this_peer_finished = False  # used by synced_gpus only
        print(__class__, "debug input ids:", torch.cat([inputs[0], special_tokens_ids]) if special_tokens_ids is not None else inputs[0])
        while True:
            ...
            if input_ids[0].shape[0] > 2:
                for j in range(batch_size):
                    for i in range(num_beams):
                        ...
                        last_index = (inputs[j] == last_id).nonzero().view(-1)
                        print(__class__, "last_id:",last_id, "last_index:", last_index)
                        if last_index.shape[0] > 0 and last_index[-1] == inputs[j].shape[0]:
                ...
                print(__class__, "last_index", last_index)
            else:
                for i in range(inputs.shape[0]):
                    target_index.append(torch.cat([inputs[i], special_tokens_ids]) if special_tokens_ids is not None else inputs[i])
            ...
            # for i in range(inputs.shape[0]):
            #     index_source = torch.cat([inputs[i], special_tokens_ids]) if special_tokens_ids is not None else inputs[i]
            ...
            # print(__class__, "new next_indices", new_next_indices)
            # print(__class__, "next_tokens", next_tokens)
            # print(__class__, "new next_tokens", new_next_tokens)
            # stateless
